...
import {
  ChevronLeft, CheckCircle, Star, Zap, Shield, ArrowRight,
  Play, AlertTriangle, Target, RefreshCw, BookOpen, Mail
} from 'lucide-react';

const INCLUDES = [
  'Custom program built around your level, weaknesses, and goals',
  'One in-depth session analysis — send footage from a full workout',
  'Form and technique breakdown from BTCALI',
  'Weakness identification specific to your planche training',
...
const PROCESS = [
  {
    ...
    desc: 'Complete the Athlete Scan below and enquire with BTCALI. Pay AUD $50 for your custom Planche Program.',
  },
  {
    step: '02',
    title: 'Receive Your Program',
    desc: 'BTCALI builds a custom program around your current level, weaknesses, and planche goals.',
  },
  {
    step: '03',
...
const FL_TUTORIALS = [
  {
    ...
    videoId: '-cGOxgIccqU',
    isShort: true,
    points: ['Externally rotate biceps', 'Protraction + depression', 'Active press, not passive lean', 'PPT + core/glute engagement'],
  },
  {
    title: 'Scapular Protraction & Retraction',
    videoId: 'QppuGF94PLc',
    isShort: true,
    points: ['Essential for planche', 'Essential for handstand', 'Shoulder health foundation'],
  },
  {
    title: 'Wrist Warm-Up',
    videoId: 'A1YPZdLyXPI',
    isShort: true,
    points: ['Do before every planche session', 'Heavy wrist loading prep', 'Discomfort okay — pain is NOT'],
  },
];
  ...
  return (
    <>
      <motion.div
        ...
        className="rounded-2xl border border-border/30 bg-card/60 overflow-hidden hover:border-primary/40 transition-all duration-300"
        onMouseEnter={e => { e.currentTarget.style.boxShadow = '0 0 20px hsl(var(--glow-primary) / 0.15)'; }}
      ...
      >
        ...
        <div className="p-4">
          ...
          <motion.button
            ...
            className="w-full py-2 rounded-xl gradient-bg-strong flex items-center justify-center gap-2 text-xs font-heading font-bold text-primary-foreground"
          >
...
export default function PlanchePrograms() {
  const handleEnquire = () => {
    const subject = encodeURIComponent('BTCALI Custom Planche Program — AUD $50');
    const body = encodeURIComponent(`Hi BTCALI,\n\nI want to enquire about the custom Planche Program (AUD $50).\n\nName:\nInstagram:\nCurrent Planche Level:\nGoals:\n\n[I have completed / plan to complete the Athlete Scan]`);
    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
        ...
        <h1 className="font-heading font-bold text-3xl sm:text-4xl mb-4">
          <span className="gradient-text">Custom Planche Program</span>
        </h1>
        <p className="text-muted-foreground font-body text-base max-w-xl mx-auto mb-6">
          A fully personalized planche program built around your current level, weaknesses, and goals — with a real session analysis included.
        </p>

        {/* Price hero */}
        <div className="inline-block glass rounded-2xl px-8 py-5 glow-border relative overflow-hidden mb-2">
          <div className="absolute inset-0 gradient-bg pointer-events-none" />
          <div className="relative">
            <p className="text-xs font-heading text-primary uppercase tracking-widest font-bold mb-1">One-Time Investment</p>
            <div className="font-heading font-bold text-5xl gradient-text">AUD $50</div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
        <div className="rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 to-accent/5 p-6">
          ...
          <p className="text-sm font-body text-foreground/80 leading-relaxed">
            You receive detailed feedback on your form, technique, muscle activations, and exactly how to perform each movement correctly.
          </p>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {PROCESS.map((p, i) => (
            ...
            >
              <div className="font-heading font-bold text-3xl gradient-text mb-2">{p.step}</div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-heading font-bold text-xl text-foreground">
            Free <span className="gradient-text">Planche Tutorials</span>
          </h2>
        </div>
      ...
      >
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-heading font-bold text-xl text-foreground">
              <span className="gradient-text">Read everything above before applying.</span>
            </h2>
          </div>
          ...
          <div className="glass rounded-xl p-4 mb-5 border border-primary/20">
            <p className="text-xs font-heading font-bold text-primary uppercase tracking-wider mb-2">Why complete the Athlete Scan first?</p>
            <p className="text-xs font-body text-muted-foreground leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <motion.button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-4 rounded-xl gradient-bg-strong text-primary-foreground font-heading font-bold text-sm glow-primary"
            >
            ...
            <Link to="/scan" className="flex-1">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.97 }}
                className="w-full flex items-center justify-center gap-2 py-4 rounded-xl glass border border-primary/40 text-foreground font-heading font-bold text-sm hover:border-primary/70 transition-all"
              >
                <ArrowRight className="w-4 h-4 text-primary" /> Start Athlete Scan First
              </motion.button>
            </Link>
          </div>
        </div>
