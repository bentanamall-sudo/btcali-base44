...
function VideoModal({ tutorial, onClose }) {
  const embedSrc = tutorial.isLong
    ? `https://www.youtube.com/embed/${tutorial.videoId}?autoplay=1&rel=0`
    : `https://www.youtube.com/embed/${tutorial.videoId}?autoplay=1&rel=0`;
        ...
        >
          <div className="flex items-center justify-between px-5 py-3 border-b border-border/30">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <a
                href={tutorial.isLong ? `https://youtu.be/${tutorial.videoId}` : `https://youtube.com/shorts/${tutorial.videoId}`}
                target="_blank"
          ...
          <div className={`relative w-full ${tutorial.isLong ? '' : ''}`} style={{ paddingBottom: tutorial.isLong ? '56.25%' : '177.78%' }}>
            <iframe
              src={embedSrc}
  ...
  return (
    <>
      <motion.div
        ...
        className={`rounded-2xl border overflow-hidden transition-all duration-300 relative ${comingSoon ? 'border-border/20 bg-card/30 opacity-60' : 'border-border/30 bg-card/60 hover:border-primary/40'}`}
      ...
      >
        ...
        <div className="p-5">
          ...
          {isAdmin && !comingSoon && tutorial.youtubeUrl && <AdminCopyOverlay url={tutorial.youtubeUrl} />}
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <div className="text-center">
          ...
          <h1 className="font-heading font-bold text-3xl sm:text-4xl mb-3">
            Free <span className="gradient-text">L-Sit To Handstand Guide</span>
          </h1>
          <p className="text-muted-foreground font-body text-base max-w-xl mx-auto">
            Master the bent arm press, develop explosive coordination, and progress toward the L-sit to handstand.
          </p>
        </div>
      </motion.div>
